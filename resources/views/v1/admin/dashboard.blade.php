...
@section('page-actions')
    <a href="{{ route('v1.admin.reports.index') }}" class="btn btn-outline-primary btn-sm">
        <i class="bi bi-graph-up me-1"></i>{{ __('messages.Reports & Analytics') }}
    </a>
@endsection
        ...
        <div class="dashboard-kpi-grid mb-4">
            @foreach ($stats as $stat)
                ...
                <div class="dashboard-kpi">
                    <div class="dashboard-kpi__icon {{ $iconClass }}">
                        <i class="bi {{ $icon }}"></i>
                    </div>
                    <div class="dashboard-kpi__label">{{ $stat['label'] }}</div>
                    ...
                    @if (!empty($stat['description']))
                        <div class="dashboard-kpi__meta">{{ $stat['description'] }}</div>
                    @endif
                    @if (!empty($stat['href']))
            ...
            <div class="col-lg-6">
                <div class="card gahez-chart-card border-0 shadow-sm h-100">
                    <div class="card-header">
                        ...
                        <p class="gahez-chart-subtitle mb-0">
                            {{ __('messages.Last 30 days') }} · {{ $charts['revenue_trend']['period_label'] ?? $charts['period_label'] ?? '' }}
                        </p>
                    </div>
                    <div class="card-body">
                        <div id="dashboardRevenueChart" class="gahez-chart"></div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6">
                <div class="card gahez-chart-card border-0 shadow-sm h-100">
                    <div class="card-header">
                        ...
                        <p class="gahez-chart-subtitle mb-0">
                            {{ __('messages.Last 30 days') }} · {{ $charts['orders_trend']['period_label'] ?? $charts['period_label'] ?? '' }}
                        </p>
                    </div>
                    <div class="card-body">
                        <div id="dashboardOrdersChart" class="gahez-chart gahez-chart--orders"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-5">
                <div class="card gahez-chart-card border-0 shadow-sm h-100">
                    ...
                    <div class="card-body">
                        <div id="dashboardPaymentChart" class="gahez-chart gahez-chart--sm"></div>
                    </div>
                </div>
            </div>
            <div class="col-md-7">
                <div class="card gahez-chart-card border-0 shadow-sm h-100">
                    ...
                    <div class="card-body">
                        <div id="dashboardTopProductsChart" class="gahez-chart gahez-chart--sm"></div>
                    </div>
                </div>
            </div>
        </div>
...
@push('scripts')
    ...
    @include('v1.admin.partials.analytics-charts-script', [
        ...
        'revenueChartId' => 'dashboardRevenueChart',
        'ordersChartId' => 'dashboardOrdersChart',
        'paymentChartId' => 'dashboardPaymentChart',
        'topProductsChartId' => 'dashboardTopProductsChart',
        'ordersChartHeight' => 300,
